...
function Header() {
    ...
    function handleList() {
        return (
            ListView.current.classList.toggle(setActive(!isActive))
        )
    }
                ...
                <nav className="navbar bg-[#8971dc]">
                    <ul className="flex justify-between items-center h-20 border-b-border-thin border-borderGrey px-6  text-sm text-white ">
                        <div className="left-container sm:p-2 sm:flex sm:items-center hidden ">
                            <li className='text-3xl font-bold cursor-pointer'>Mercari</li>

                            <li className="flex items-center px-5 hover:cursor-pointer relative  group" onClick={handleList}>
